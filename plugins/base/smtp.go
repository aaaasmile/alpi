...
func (msg *OutgoingMessage) WriteTo(w io.Writer) (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("failed to create mail writer: %v", err)
	}
	...
	if err != nil {
		return 0, fmt.Errorf("failed to create text part: %v", err)
	}
	defer tw.Close()

	if _, err := io.WriteString(tw, msg.Text); err != nil {
		return 0, fmt.Errorf("failed to write text part: %v", err)
	}

	if err := tw.Close(); err != nil {
		return 0, fmt.Errorf("failed to close text part: %v", err)
	}
	...
	if err := mw.Close(); err != nil {
		return 0, fmt.Errorf("failed to close mail writer: %v", err)
	}
...
func sendMessage(c *smtp.Client, msg *OutgoingMessage) error {
	...
	if _, err := stripped.WriteTo(w); err != nil {
