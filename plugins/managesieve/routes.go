...
func registerRoutes(p *plugin) {
	p.GET("/filters", func(ctx *websrv.Context) error {
	...
	p.GET("/filters/:name", func(ctx *websrv.Context) error {
		c, err := p.connect(ctx.Session)
		if err != nil {
			return err
		}
	...
	updateFilter := func(ctx *websrv.Context) error {
		c, err := p.connect(ctx.Session)
		if err != nil {
			return err
		}
	...
	renameFilter := func(ctx *websrv.Context) error {
		c, err := p.connect(ctx.Session)
		if err != nil {
			return err
		}
	...
	p.POST("/filters/activate", func(ctx *websrv.Context) error {
		c, err := p.connect(ctx.Session)
		if err != nil {
			return err
		}
	...
	p.POST("/filters/delete", func(ctx *websrv.Context) error {
		c, err := p.connect(ctx.Session)
		if err != nil {
			return err
		}
